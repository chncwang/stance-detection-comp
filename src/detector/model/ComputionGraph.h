...
public:
    ...
    std::vector<UniNode> _tfidf_uninodes;
...
public:
    ...
    void createNodes(int length_upper_bound) {
        ...
        _tfidf_uninodes.resize(length_upper_bound);
    }
...
public:
    void initial(Graph *pcg, ModelParams &model, HyperParams &opts) {
        ...
        for (auto &n : _tfidf_uninodes) {
            n.init(opts.hiddenSize, opts.dropProb);
            n.setParam(&model._uni_params);
        }
        ...
        _tfidf_pool.init(opts.hiddenSize, -1);
...
public:
    ...
    inline void forward(const Feature &feature, bool bTrain = false) {
        ...
        for (int i = 0; i < feature.m_target_tfidf_words->size(); ++i) {
            _tfidf_uninodes.at(i).forward( _graph,&_target_tfidf_nodes.at(i));
        }

        _tfidf_pool.forward(_graph, toPointers<UniNode, Node>(_tfidf_uninodes, feature.m_target_tfidf_words->size()));
