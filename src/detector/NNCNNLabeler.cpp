...
void printBalancedStancesCounts(const std::vector<Example> &examples, const std::vector<int> &indexes) {
    ...
    for (int index : indexes) {
        counts.at(examples.at(index).m_stance)++;
    }
    std::cout << "trainset favor count: " << counts.at(Stance::FAVOR) << std::endl;
    std::cout << "trainset against count: " << counts.at(Stance::AGAINST) << std::endl;
    std::cout << "trainset none count: " << counts.at(Stance::NONE) << std::endl;
}
...
void Classifier::train(const string &trainFile, const string &devFile,
    ...
    for (int iter = 0; iter < m_options.maxIter; ++iter) {
        ...
        std::vector<int> indexes = getClassBalancedIndexes(trainExamples, m_options.getRatios());
        printBalancedStancesCounts(trainExamples, indexes);
